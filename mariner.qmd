...
## Prepare the data {.nonincremental}
...
## Transform _de novo_ TADs for visualization
...
## Reassemble TADs
anchor1 <- anchors(deNovo, "first")
anchor2 <- anchors(deNovo, "second")
deNovoTads <- 
  GRanges(seqnames = seqnames(anchor1),
          ranges = IRanges(start = start(anchor1),
                           end = end(anchor2)))
head(deNovoTads)
```
...
## Annotate TAD domains
annoDomains(plot = bottom,
            data = deNovoTads,
            linecolor = "red",
            params = p)
